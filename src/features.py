...
def engineer_process_features(df):
    ...
    # 1. Dynamically create column lists (1 to 7)
    ...
    # 2. Calculate the "Process Averages" across all 7 columns
    ...
    for col in variables_to_process:
        if col in df.columns:
            # Lags (Historical snapshots)
            df[f'{col}_lag1'] = df[col].shift(1)
            df[f'{col}_lag3'] = df[col].shift(3)

            # Rolling (Capturing trends)
